# 実装担当への引き継ぎ / 第5版
R2aではPR #4の8武器共通ルール・検証済み操作入口・敵4役割の判断基盤を保持し、PR #5の4種の補給・連続移動・運搬・砲撃の操作確認画面を追加した。PR #6では ../../RULE_ENGINE.md の接続契約に従い、src/simulation/r2b-bridge.ts で固定座標の最初の接触と世代付き広場突破を検証済み入力へ変換した。PR #7では `BattleIntent.bridge` を物理 `stepBattle` の同一更新へ接続し、共通 `stepWorld` の状態とイベントを一度だけ取り込む。PR #8ではダッシュの人物接触を1損傷・36更新無敵・選択中荷物一個の落下・0.6床の押し戻しへ接続し、PR #9〜#10では突進実行器と設備接触・停止復旧、PR #11では内部兵の城内目標・局所防衛・退避を物理固定更新へ接続した。共通ルールは src/simulation/battle.ts、物理操作確認は src/simulation/physical-battle.ts で、公開APIの後者は physicalBattle 名前空間。物理側が時計・乱数・イベント履歴を所有し、状態・命令・座標単位を不正に混用しない。実広場座標、跨城侵入、補給効果・修理・共通ルール側の敵判断、観戦画面を含む通常戦は未完了。R2全体の完了ではない。
