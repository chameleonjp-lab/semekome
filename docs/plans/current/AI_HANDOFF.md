...
今回のDraft PRでは、標準配分（標準3・重量2・防護2・高速1）の8個周期を `src/logistics/supply-schedule.ts` の決定的な共通生成器へ移し、物理側の4補給口が陣営ごとに一つのカーソルを成功時だけ消費するようにした。停止・設備停止・床上限・共通遷移拒否では順番を進めず、再開後に待ち時間分をまとめて生成しない。実際の床物体と固定座標は物理側だけが作り、共通 `stepWorld` との二重生成はない。共通WorldStateの自動床生成、補給選択、全面配送、R3全体、通常戦の終局、観戦・結果・ランキングは未完了である。
