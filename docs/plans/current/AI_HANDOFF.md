...
今回のDraft PRでは、直前の共通 `SupplySchedule` を `BattleState.supply` から読み、共通 `stepBattle` が陣営ごとの4補給口について `spawn_supply` を自動生成する。停止・設備停止・床上限（部屋16個）・由来グループ上限・共通遷移拒否では順番を進めず、空き床を補給口に近い安定順で選び、受け入れられたケースだけID・生成元グループ・次回時刻・カーソルを更新する。物理操作確認版の補給生成は別経路のままにして二重生成を避ける。補給選択、全面配送、R3全体、通常戦の終局、観戦・結果・ランキングは未完了である。
